...
Tiktok = requests.get('https://zmatrixtool.x10.mx/Api/Info/Tiktok.php').text
Youtube = requests.get('https://zmatrixtool.x10.mx/Api/Info/Youtube.php').text
def banner():
    os.system('cls' if os.name == 'nt' else 'clear')
    version = requests.get('https://zmatrixtool.x10.mx/Api/Version.php').text
    copyright_lines = [
        "CopyRight: ©Z-Matrix",
        f"Phiên bản: {version}"
    ]
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=30) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                print(f'')
    print(f"{luc}Spam thành công lần :{cam}", i)
    for j in range(4, 0, -1):
        Write.Print(f'[Z-Matrix] Đang chống block... Chạy lại sau {i} giây\r', Colors.rainbow, interval=0.0001)
        sleep(1)
        print(' ' * 80, end='\r')  # Xoá dòng trước để không bị đè
if __name__ == "__main__":
    banner()
    print(f'{hong}Lưu ý: Đây chỉ là bản lỏ.')
    phone = input(f"{trang}Nhập số điện thoại: {vang}")
    count = int(input(f"{trang}Nhập số lần spam:{vang} "))
